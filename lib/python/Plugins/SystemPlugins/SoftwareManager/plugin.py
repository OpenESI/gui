...
from .BackupRestore import InitConfig as BackupRestore_InitConfig, BackupSelection, BackupScreen, RestoreScreen, getBackupPath, getOldBackupPath, getBackupFilename, RestoreMyDarknessHD, RestoreMenu
...
class UpdatePlugin(Screen):
	...
	def checkTrafficLight(self):
		...
		try:
			...
			message = _("The status of the current update could not be checked because https://www.openesi.eu could not be reached for some reason") + "\n"
			default = False
	...
	def restoreDarknessHD(self):
		try:
			if config.skin.primary_skin.value == "DarknessHD/skin.MySkin.xml" and not isfile("/usr/share/enigma2/DarknessHD/skin.MySkin.xml"):
				self.session.openWithCallback(self.restoreDarknessHDCallback, RestoreMyDarknessHD)
			elif config.skin.primary_skin.value == "DarknessHD/skin.MySkin.xml" and config.plugins.MyDarknessHDLiteOther.EHDenabled.value != "0":
				from Plugins.Extensions.MyDarknessHDLite.ActivateSkinSettings import ActivateSkinSettings
				ActivateSkinSettings().RefreshIcons()
				self.restoreDarknessHDCallback()
			else:
				self.restoreDarknessHDCallback()
		except:
			self.restoreDarknessHDCallback()

	def restoreDarknessHDCallback(self, ret=None):
