...
class IPKGMenu(Screen):
	skin = """
		<screen name="IPKGMenu" position="center,center" size="560,400" resolution="1280,720">
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/buttons/red.png" position="0,0" size="140,40" alphatest="on" />
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/buttons/green.png" position="140,0" size="140,40" alphatest="on" />
	...
	def __init__(self, session):
		...
		self.setTitle(_("Seleziona Sorgente Aggiornamento da Modificare"))
		self["key_red"] = StaticText(_("Chiudi"))
		self["key_green"] = StaticText(_("Modifica"))
		self.sel = []
		self.val = []
		self.entry = False
		self.exe = False
...
class IPKGSource(Screen):
	skin = """
		<screen name="IPKGSource" position="center,center" size="560,80" title="Modifica URL sorgente aggiornamento" resolution="1280,720">
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/buttons/red.png" position="0,0" size="140,40" alphatest="on" />
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/buttons/green.png" position="140,0" size="140,40" alphatest="on" />
			<widget source="key_red" render="Label" position="0,0" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#9f1313" transparent="1" />
			<widget source="key_green" render="Label" position="140,0" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#1f771f" transparent="1" />
	...
	def __init__(self, session, configfile=None):
		Screen.__init__(self, session)
		self.setTitle(_("Modifica URL Sorgente Aggiornamento"))
		...
		self["key_red"] = StaticText(_("Annulla"))
		self["key_green"] = StaticText(_("Salva"))
		if (y >= 720):
...
class IpkgInstaller(Screen):
	skin = """
		<screen name="IpkgInstaller" position="center,center" size="550,450" title="Installa estensioni" resolution="1280,720">
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/buttons/red.png" position="0,0" size="140,40" alphatest="on" />
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/buttons/green.png" position="140,0" size="140,40" alphatest="on" />
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/buttons/yellow.png" position="280,0" size="140,40" alphatest="on" />
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/buttons/blue.png" position="420,0" size="140,40" alphatest="on" />
			<widget source="key_red" render="Label" position="0,0" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#9f1313" transparent="1" />
			<widget source="key_green" render="Label" position="140,0" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#1f771f" transparent="1" />
			<widget source="key_yellow" render="Label" position="280,0" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#a08500" transparent="1" />
			<widget source="key_blue" render="Label" position="420,0" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#18188b" transparent="1" />
			<widget name="list" position="5,50" size="540,360" />
			<ePixmap pixmap="/usr/share/enigma2/Maxy-FHD/div-h.png" position="0,410" zPosition="10" size="560,2" transparent="1" alphatest="on" />
	...
	def __init__(self, session, list):
		...
		if len(list):
			p = list[0].rfind("/")
			title = list[0][:p]
			self.title = ("%s %s %s") % (_("Installa estensioni"), _("da"), title)
		for listindex in range(len(list)):
			self.selectionList.addSelection(list[listindex][p + 1:], list[listindex], listindex, False)
		self.selectionList.sort()
		self["key_red"] = StaticText(_("Chiudi"))
		self["key_green"] = StaticText(_("Installa"))
		self["key_yellow"] = StaticText()
		self["key_blue"] = StaticText(_("Inverti"))
		self["introduction"] = StaticText(_("Premi OK per selezionare/deselezionare."))
...
def filescan(**kwargs):
	...
	], name="Ipkg", description=_("Installa estensioni."), openfnc=filescan_open)


class BackupHelper(Screen):
	skin = """
		<screen name="BackupHelper" position="0,0" size="1,1" title="Software Manager">
		</screen>"""
	...
	def doAction(self):
		...
		if self.args == 1:
			...
			if isfile(self.fullbackupfilename):
				self.session.openWithCallback(self.startRestore, MessageBox, _("Sei sicuro di voler ripristinare il backup?\nIl tuo receiver si riavvierà dopo il ripristino!"), default=False)
				doClose = False
			else:
				self.session.open(MessageBox, _("Nessun backup trovato!"), MessageBox.TYPE_INFO, timeout=10)
		elif self.args == 3:
			try:
				...
				self.session.open(MessageBox, _("Scusa, %s non è installato!") % ("MediaScanner"), MessageBox.TYPE_INFO, timeout=10)
		elif self.args == 5:
			self.session.open(BackupSelection, title=_("File/cartelle predefiniti per il backup"), configBackupDirs=config.plugins.configurationbackup.backupdirs_default, readOnly=True, mode="backupfiles")
		elif self.args == 6:
			self.session.open(BackupSelection, title=_("File/cartelle aggiuntivi per il backup"), configBackupDirs=config.plugins.configurationbackup.backupdirs, readOnly=False, mode="backupfiles_addon")
		elif self.args == 7:
			self.session.open(BackupSelection, title=_("File/cartelle da escludere dal backup"), configBackupDirs=config.plugins.configurationbackup.backupdirs_exclude, readOnly=False, mode="backupfiles_exclude")
		if doClose:
			self.close()
	...
	def backupDone(self, retval=None):
		#message = _("Backup completato.") if retval else _("Backup fallito.")
