...
def InitConfig():
	...
	BACKUPFILES = ['/etc/enigma2/', '/etc/CCcam.cfg', '/usr/keys/',
		...
		'/usr/share/enigma2/DarknessHD/skinparts/',
...
class RestoreScreen(Screen, ConfigListScreen):
	...
	def userRestoreScript(self, ret=None):
		...
		if startSH:
			self.session.openWithCallback(self.restoreDarknessHDSkin, Console, title=_("Running Myrestore script, Please wait ..."), cmdlist=[startSH], closeOnSuccess=True)
		else:
			self.restoreDarknessHDSkin()
	...
	def restoreDarknessHDSkin(self, ret=None):
		configfile.load()
		configfile.save()
		try:
			...
			s = f.read()
			f.close()
			restore = "config.skin.primary_skin=DarknessHD/skin.MySkin.xml" in s
		except:
			restore = False
		if restore:
			self.session.openWithCallback(self.rebootSYS, RestoreMyDarknessHD)
		else:
			self.rebootSYS()
...
class RestoreMyDarknessHD(Screen):

	def __init__(self, session):
		Screen.__init__(self, session)
		self.setTitle(_("Restore DarknessHD Settings"))
		skin = """
			<screen name="RestoreDarknessHD" position="center,center" size="600,100" title="Restore DarknessHD Settings">
	...
	def restoreSkin(self):
		try:
			...
			if result:
				infotext = ({1: _("Unknown Error creating Skin.\nPlease check after reboot MyDarknessHDLite-Plugin and apply your settings."),
			...
			self.session.openWithCallback(self.checkSkinCallback, MessageBox, _("Error creating DarknessHD-Skin.\nPlease check after reboot MyDarknessHDLite-Plugin and apply your settings."), MessageBox.TYPE_ERROR, timeout=30)
