...
def InitConfig():
	...
	BACKUPFILES = ["/etc/enigma2/", "/etc/CCcam.cfg", "/usr/keys/",
		...
		"/usr/share/enigma2/Maxy-FHD/skinparts/",  # Modificato per Maxy-FHD
...
class RestoreScreen(ConfigListScreen, Screen):
	...
	def userRestoreScript(self, ret=None):
		...
		if scriptPath:
			self.session.openWithCallback(self.restartGUI, Console, title=_("Running Myrestore script, Please wait ..."), cmdlist=[scriptPath], closeOnSuccess=True, showScripts=False)
		else:
			self.restartGUI()
