...
config.plugins.esiteam = ConfigSubsection()
config.plugins.esiteam.dropmode = ConfigSelection(default = '3', choices = [
		('1', _("free pagecache")),
...
class ESISettings(Screen):
	skin = """
<screen name="ESISettings" position="70,35" size="1150,650">
...
"itemHeight": 40
	...
	def KeyOk(self):
		...
		self.sel = self.sel[2]
		if self.sel == 0:
			from Plugins.Extensions.ESIteam.MountManager import HddMount
			self.session.open(HddMount)
		elif self.sel == 1:
			from Plugins.Extensions.ESIteam.EsiEparted import Ceparted
			self.session.open(Ceparted)
		elif self.sel == 2:
			from Plugins.Extensions.ESIteam.EsiSwapManager import Swap
			...
			self.session.open(ESImemoria)
		elif self.sel == 12:
			from Screens.CronTimer import CronTimers
			self.session.open(CronTimers)
		elif self.sel == 13:
			from Plugins.Extensions.ESIteam.EsiRestartNetwork import RestartNetwork
			self.session.open(RestartNetwork)
		elif self.sel == 14:
			from Screens.CCcamInfo import CCcamInfoMain
			self.session.open(CCcamInfoMain)
		elif self.sel == 15:
			from Plugins.Extensions.ESIteam.Esisundtek import SundtekControlCenter
	...
	def updateList(self):
		self.list = []
		mypath = '/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/icons/'
...
class EsiEpgPanel(Screen):
	skin = """
	<screen position="center,center" size="600,400" title="OpenESI EPG Panel">
	...
	def KeyOk(self):
		...
		if sel:
			if sel == "EPGSettings":
				...
				from Plugins.Extensions.ESIteam.EsiEpg import ESIepgScreen
				self.session.open(ESIepgScreen)
			elif sel == "CrossEPG":
...
class ESImemoria(ConfigListScreen, Screen):
	skin = """
<screen name="ESImemoria" position="center,160" size="750,370" title="Liberar Memoria">
	...
	<ePixmap position="10,358" zPosition="1" size="165,2" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/buttons/red150x30.png" alphatest="blend" />
	...
	<ePixmap position="175,358" zPosition="1" size="165,2" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/buttons/green150x30.png" alphatest="blend" />
	...
	<ePixmap position="340,358" zPosition="1" size="195,2" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/buttons/yellow150x30.png" alphatest="blend" />
	...
	def __init__(self, session):
		...
		if config.plugins.esiteam.dropmode:
			try:
				self.list.append(getConfigListEntry(_('Select free memory mode'), config.plugins.esiteam.dropmode))
			except:
				pass
	...
	def ClearNow(self):
		self.iConsole.ePopen('echo %s > /proc/sys/vm/drop_caches' % config.plugins.esiteam.dropmode.value, self.Finish)
...
class EsiNetBrowser(Screen):
	skin = """
	<screen position="center,center" size="800,520" title="OpenESI - Seleccione la interfaz de red">
		<widget source="list" render="Listbox" position="10,10" size="780,460" scrollbarMode="showOnDemand" >
			<convert type="StringList" />
		</widget>
			<ePixmap pixmap="/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/buttons/red150x30.png" position="200,480" size="150,30" alphatest="on" />
		<ePixmap pixmap="/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/buttons/yellow150x30.png" position="440,480" size="150,30" alphatest="on" />
