...
config.plugins.esiteam.swapautostart = ConfigYesNo(default=False)
...
def SwapAutostart(reason, session=None, **kwargs):
	global startswap
	if reason == 0:
		if config.plugins.esiteam.swapautostart.value:
...
class Swap(Screen):
	...
	def getSwapDevice(self):
		...
		if path.exists('/etc/rcS.d/S98SwapManager'):
			...
			config.plugins.esiteam.swapautostart.value = True
			config.plugins.esiteam.swapautostart.save()
	...
	def updateSwap2(self, result=None, retval=None, extra_args=None):
		...
		if config.plugins.esiteam.swapautostart.value and self.swap_place:
			self['autostart_off'].hide()
			self['autostart_on'].show()
		else:
			config.plugins.esiteam.swapautostart.setValue(False)
			config.plugins.esiteam.swapautostart.save()
			configfile.save()
			self['autostart_on'].hide()
			self['autostart_off'].show()
	...
	def autoSsWap(self):
		if self.swap_place:
			if config.plugins.esiteam.swapautostart.value:
				...
				config.plugins.esiteam.swapautostart.setValue(True)
				if fileExists('/etc/init.d/SwapManager'):
