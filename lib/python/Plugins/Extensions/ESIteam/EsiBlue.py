...
from Components.Converter.EsiExtraInfo import EsiExtraInfo
...
class ESIBluePanel(Screen):
	skin = """
<screen name="ESIBluePanel" position="center,center" size="1150,650">
...
<widget source="session.CurrentService" render="Label" position="256,25" size="240,24" font="Regular; 15" backgroundColor="transpBlack" halign="left" transparent="1" zPosition="5">
		<convert type="EsiExtraInfo">CryptoNameCaid</convert>
</widget>
<widget source="session.CurrentService" render="Label" position="352,25" size="240,24" font="Regular; 15" backgroundColor="transpBlack" halign="left" transparent="1" zPosition="5">
		<convert type="EsiExtraInfo">E-C-N</convert>
</widget>
...
<ePixmap position="25,55" size="510,255" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/menu/bp_deko.png" zPosition="1" alphatest="on" />
...
<widget source="session.CurrentService" render="Label" position="40,293" size="505,380" font="Regular; 16" halign="left" backgroundColor="transpBlack" transparent="1" zPosition="2">
		<convert type="EsiExtraInfo">Ecmtext</convert>
</widget>
<widget source="session.CurrentService" render="Label" position="40,415" size="505,380" font="Regular; 14" halign="left" backgroundColor="transpBlack" transparent="1" zPosition="2">
		<convert type="EsiExtraInfo">PIDtext</convert>
</widget>
<widget source="session.CurrentService" render="Label" position="40,490" size="505,380" font="Regular; 13" halign="left" backgroundColor="transpBlack" transparent="1" zPosition="2">
		<convert type="EsiExtraInfo">InfoPeer</convert>
</widget>
</screen>"""
	...
	def populate_List(self):
		...
		if fileExists('/etc/EsiCamConf') == False:
			out = open('/etc/EsiCamConf', 'w')
	...
	def keyOk(self):
		...
		inme = open('/etc/EsiCamConf', 'r')
		out = open('/etc/EsiCamConf.tmp', 'w')
		...
		out.close()
		inme.close()
		os_rename('/etc/EsiCamConf.tmp', '/etc/EsiCamConf')
		out = open('/etc/CurrentEsiCamName', 'w')
		out.write(self.sel)
		out.close()
		cmd = 'cp -f ' + self.newcam + ' /usr/bin/StartEsiCam'
		system(cmd)
		cmd = 'STOP_CAMD,' + self.defaultcam
		self.sendtoEsi_sock(cmd)
		self.session.openWithCallback(self.keyOk2, startstopCam, self.defCamname, _('Stopping'))

	def keyOk2(self):
		os.system('/usr/bin/StartEsiCam stop 2>/dev/null')
	...
	def sendtoEsi_sock(self, data):
		client_socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
		if fileExists('/tmp/OpenESI.socket'):
			try:
				client_socket.connect('/tmp/OpenESI.socket')
			except:
				pass
		else:
			os.system('start-stop-daemon -S -b -x /usr/bin/openesisocker 2>/dev/null')
			os.system('/etc/init.d/openesisocker stop 2>/dev/null')
			os.system('/etc/init.d/openesisocker start 2>/dev/null')
			self.session.open(MessageBox, _("We have not found OpenESI.socket and we have generated the new temporay Socket..."), MessageBox.TYPE_INFO, timeout = 8)
			client_socket.connect('/tmp/OpenESI.socket')
		client_socket.send(data)
		client_socket.close()

	def keyBlue(self):
		from EsiSet import ESISettings
		self.session.open(ESISettings)

	def keyGreen(self):
		from Plugins.Extensions.ESIteam.EsiPlugin import ESIPluginPanel
		self.session.open(ESIPluginPanel)

	def keyRed(self):
		from EsiServ import ESIServices
		self.session.open(ESIServices)

	def keyYellow(self):
		from EsiAbout import EsisysInfo
		self.session.open(EsisysInfo)
...
class ESIBp:
	...
	def __init__(self):
		self['ESIBp'] = ActionMap(['InfobarExtensions'], {'ESIBpshow': self.showESIBp})

	def showESIBp(self):
		self.session.openWithCallback(self.callNabAction, ESIBluePanel)
