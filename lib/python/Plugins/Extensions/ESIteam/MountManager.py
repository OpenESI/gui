...
class HddMount(Screen):
	...
	def buildMy_rec(self, device):
		...
		d2 = device
		name = 'USB: '
		mypixmap = '/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/icons/dev_usbstick.png'
		if device2.startswith('mmcblk'):
			model = file('/sys/block/' + device2 + '/device/name').read()
			mypixmap = '/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/icons/dev_mmc.png'
			name = 'MMC: '
		else:
		...
		if devicetype.find('/devices/pci') != -1 or devicetype.find('ahci') != -1:
			name = _("HARD DISK: ")
			mypixmap = '/usr/lib/enigma2/python/Plugins/Extensions/ESIteam/images/icons/dev_hdd.png'
		name = name + model
