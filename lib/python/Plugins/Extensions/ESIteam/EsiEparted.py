...
class Ceparted(Screen):
	skin = """<screen position="center,center" size="600,200" title="eParted OpenESI">
...
class AddPart(Screen, ConfigListScreen):
	...
	def __init__(self, session, maxsize, unit, countpart):
		...
		if pathExists("/sbin/mkfs.msdos"):
			menu.append("fat16")
		config.plugins.esiteam.fs = NoSave(ConfigSelection(default = default, choices = menu))
		config.plugins.esiteam.size = NoSave(ConfigInteger(default=maxsize, limits=[1,maxsize]))

		list = []
		if countpart<4:
			list.append(getConfigListEntry(_("size in %s (max %d %s):") %(unit, maxsize, unit), config.plugins.esiteam.size))
		list.append(getConfigListEntry(_("filesystem:"), config.plugins.esiteam.fs))
	...
	def keySave(self):
		if config.plugins.esiteam.size.value>0:
			self.close((config.plugins.esiteam.size.value, config.plugins.esiteam.fs.value))
