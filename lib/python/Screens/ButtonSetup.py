...
def getButtonSetupKeys():
    return [(_('Red'), 'red', 'Plugins/Extensions/WeatherPlugin/1'),
     ...
     (_('Stop'), 'stop', ''),
     (_('Pause'), 'pause', ''),
     (_('Rewind'), 'rewind', 'Infobar/seekRwdVod'),
     (_('Fastforward'), 'fastforward', 'Infobar/seekFwdVod'),
...
class InfoBarButtonSetup:
    ...
    def __init__(self):
        self.ButtonSetupKeys = getButtonSetupKeys()
        self['ButtonSetupButtonActions'] = helpableButtonSetupActionMap(self, 'ButtonSetupActions', dict(((x[1], (self.ButtonSetupGlobal, boundFunction(self.getHelpText, x[1]))) for x in self.ButtonSetupKeys)), -10)
        self.longkeyPressed = False
