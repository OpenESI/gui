	...
	def keySaveConfirm(self, ret = False):
		if ret == True:
			...
			if num_configured_if >= 1:
				if self.iface in iNetwork.getConfiguredAdapters() or (iNetwork.self.iface in onlyWoWifaces and iNetwork.onlyWoWifaces[self.iface] is True):
...
class AdapterSetupConfiguration(Screen, HelpableScreen):
	...
	def WlanStatusClosed(self, *ret):
		if ret is not None and len(ret):
			...
			self.updateStatusbar()
			if iNetwork.getAdapterAttribute(self.iface, "up") is True and iNetwork.self.iface in onlyWoWifaces and iNetwork.onlyWoWifaces[self.iface] is True:
				iNetwork.deactivateInterface(self.iface, self.deactivateInterfaceCB)
