...
def getAboutText():
	...
	AboutText += _("Firmware:\t %s") % openESI + str(getImageVersion()) + "\n"
...
class About(Screen):
	...
	def populate(self):
		self["lab1"] = StaticText(_("Support:\t https://www.openesi.eu"))
		model = None
...
class AboutSummary(Screen):
	def __init__(self, session, parent):
		...
		self["selected"] = StaticText("openESI:" + str(getImageVersion()))
...
class CommitInfo(Screen):
	def __init__(self, session):
		...
		self.projects = [
			...
			("openesi-settings", "Settings OpenESI (Collection channel settings)."),
			("openesi-plugins", "OpenESI Plugins"),
			("openesi-skins", "OpenESI Skins"),
			("openesi-tuxbox-common", "Tuxbox Common (list of known transponders)"),
	...
	def readGithubCommitLogs(self):
		url = 'https://api.github.com/repos/OpenESI/%s/commits' % self.projects[self.project][0]
		commitlog = ""
