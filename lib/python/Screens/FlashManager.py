...
from os import W_OK, access, listdir, major, makedirs, minor, mkdir, sep, stat, stESIfs, unlink, walk
...
FEED_URLS = [
	("openesi", "https://images.mynonpublic.com/openesi/json/%s", "BoxName"),
...
class FlashManager(Screen, HelpableScreen):
	...
	def getImagesList(self):
		...
		if not self.imagesList:
			...
			if index is None:
				feedURL = "https://images.mynonpublic.com/openesi/json/%s"
	...
	def getImagesListCallback(self, retVal=None):  # The retVal argument absorbs the unwanted return value from MessageBox.
		if self.imageFeed != "openesi":
			self.keyDistributionCallback("openesi")  # No images can be found for the selected distribution so go back to the openesi default.
	...
	def keyDistributionCallback(self, distribution):
		if distribution:
			...
			# TRANSLATORS: The variable is the name of a distribution.  E.g. "openesi".
			self.setTitle(_("Flash Manager - %s Images") % self.imageFeed)
			self.imagesList = {}
			self.expanded = []
			self.setIndex = 0
