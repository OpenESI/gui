...
class RunSoftwareUpdate(Screen, HelpableScreen):
	...
	def restoreDarknessHD(self):  # TODO: call this only after DarknessHD update / move this to DarknessHD Plugin
		try:
			if config.skin.primary_skin.value == "DarknessHD/skin.MySkin.xml":
				if not exists("/usr/share/enigma2/DarknessHD/skin.MySkin.xml"):
					from Plugins.SystemPlugins.SoftwareManager.BackupRestore import RestoreMyDarknessHD
					self.session.openWithCallback(self.restoreDarknessHDCallback, RestoreMyDarknessHD)
					return
				elif config.plugins.MyDarknessHDLiteOther.EHDenabled.value != '0':
