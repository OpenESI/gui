...
class SkinSelectorBase:
	...
	DarknessHD_MYSKIN = "DarknessHD/skin.MySkin.xml"
	...
	def restartGUI(self, answer):
		if answer is True:
			if isinstance(self, LcdSkinSelector):
				...
				try:
					if self.config.value == self.DarknessHD_MYSKIN:
				...
				if self.skinfile == "DarknessHD/skin.xml" and isfile(resolveFilename(SCOPE_GUISKIN, self.DarknessHD_MYSKIN)):
					self.config.value = self.DarknessHD_MYSKIN
				self.config.save()
