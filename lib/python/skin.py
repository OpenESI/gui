...
import xml.etree.ElementTree as xml.etree.cElementTree
import os
...
constant_widgets = {}
variables = {}
DEFAULT_SKIN = "Maxy-FHD/skin.xml"
...
if fileExists('/etc/.restore_skins'):
    ...
    for skin in sorted(glob.glob('/usr/lib/enigma2/python/Plugins/Extensions/*/ActivateSkinSettings.py*')):
        try:
            ...
            if skinpath == lastpath or ext not in '.pyc':
...
def skinExists(skin = False):
    if not skin or not isinstance(skin, str):
...
try:
    if config.skin.primary_skin.value == "Maxy-FHD/skin.xml" or config.skin.primary_skin.value == DEFAULT_SKIN:
        cachemenu()
except:
...
class AttributeParser:
    ...
    def orientation(self, value): # used by eSlider
        try:
            ...
            print("orientation must be either orVertical or orHorizontal!, not %s. Please contact the skin's author!" % value)
