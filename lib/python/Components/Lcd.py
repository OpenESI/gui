...
def InitLcd():
    ...
    if detected:
        ...
        if SystemInfo["LEDButtons"]:
            def setLEDbuttons(configElement):
                open(SystemInfo["LEDButtons"], "w").write("1" if configElement.value else "0")
            config.usage.ledButtons = ConfigYesNo(default=False)
            config.usage.ledButtons.addNotifier(setLEDbuttons)

        if SystemInfo["PowerLED"]:
            def setPowerLED(configElement):
                if configElement.value == "off":
                    open(SystemInfo["PowerLED"], "w").write("0")
                elif configElement.value == "on":
                    open(SystemInfo["PowerLED"], "w").write("1")
                elif configElement.value == "blink":
                    open(SystemInfo["PowerLED"], "w").write("2")
                elif configElement.value == "blinkblue":
                    open(SystemInfo["PowerLED"], "w").write("3")
            config.usage.powerLED = ConfigSelection(default="on", choices=[("off", _("Off")), ("on", _("On")), ("blink", _("Blink")), ("blinkblue", _("Blink blue"))])
            config.usage.powerLED.addNotifier(setPowerLED)

        if SystemInfo["StandbyLED"]:
            def setStandbyLED(configElement):
                if configElement.value == "off":
                    open(SystemInfo["StandbyLED"], "w").write("0")
                elif configElement.value == "on":
                    open(SystemInfo["StandbyLED"], "w").write("1")
                elif configElement.value == "blink":
                    open(SystemInfo["StandbyLED"], "w").write("2")
                elif configElement.value == "blinkblue":
                    open(SystemInfo["StandbyLED"], "w").write("3")
            config.usage.standbyLED = ConfigSelection(default="on", choices=[("off", _("Off")), ("on", _("On")), ("blink", _("Blink")), ("blinkblue", _("Blink blue"))])
            config.usage.standbyLED.addNotifier(setStandbyLED)

        if SystemInfo["SuspendLED"]:
            def setSuspendLED(configElement):
                if configElement.value == "off":
                    open(SystemInfo["SuspendLED"], "w").write("0")
                elif configElement.value == "on":
                    open(SystemInfo["SuspendLED"], "w").write("1")
                elif configElement.value == "blink":
                    open(SystemInfo["SuspendLED"], "w").write("2")
                elif configElement.value == "blinkblue":
                    open(SystemInfo["SuspendLED"], "w").write("3")
            config.usage.suspendLED = ConfigSelection(default="on", choices=[("off", _("Off")), ("on", _("On")), ("blink", _("Blink")), ("blinkblue", _("Blink blue"))])
            config.usage.suspendLED.addNotifier(setSuspendLED)

        if SystemInfo["Power4x7On"]:
            def setPower4x7On(configElement):
                if configElement.value == "off":
                    open(SystemInfo["Power4x7On"], "w").write("0")
                elif configElement.value == "on":
                    open(SystemInfo["Power4x7On"], "w").write("1")
                elif configElement.value == "blink":
                    open(SystemInfo["Power4x7On"], "w").write("2")
                elif configElement.value == "blinkblue":
                    open(SystemInfo["Power4x7On"], "w").write("3")
            config.usage.power4x7On = ConfigSelection(default="on", choices=[("off", _("Off")), ("on", _("On")), ("blink", _("Blink")), ("blinkblue", _("Blink blue"))])
            config.usage.power4x7On.addNotifier(setPower4x7On)

        if SystemInfo["Power4x7Standby"]:
            def setPower4x7Standby(configElement):
                if configElement.value == "off":
                    open(SystemInfo["Power4x7Standby"], "w").write("0")
                elif configElement.value == "on":
                    open(SystemInfo["Power4x7Standby"], "w").write("1")
                elif configElement.value == "blink":
                    open(SystemInfo["Power4x7Standby"], "w").write("2")
                elif configElement.value == "blinkblue":
                    open(SystemInfo["Power4x7Standby"], "w").write("3")
            config.usage.power4x7Standby = ConfigSelection(default="on", choices=[("off", _("Off")), ("on", _("On")), ("blink", _("Blink")), ("blinkblue", _("Blink blue"))])
            config.usage.power4x7Standby.addNotifier(setPower4x7Standby)

        if SystemInfo["Power4x7Suspend"]:
            def setPower4x7Suspend(configElement):
                if configElement.value == "off":
                    open(SystemInfo["Power4x7Suspend"], "w").write("0")
                elif configElement.value == "on":
                    open(SystemInfo["Power4x7Suspend"], "w").write("1")
                elif configElement.value == "blink":
                    open(SystemInfo["Power4x7Suspend"], "w").write("2")
                elif configElement.value == "blinkblue":
                    open(SystemInfo["Power4x7Suspend"], "w").write("3")
            config.usage.power4x7Suspend = ConfigSelection(default="on", choices=[("off", _("Off")), ("on", _("On")), ("blink", _("Blink")), ("blinkblue", _("Blink blue"))])
            config.usage.power4x7Suspend.addNotifier(setPower4x7Suspend)

        if SystemInfo["LcdLiveTVMode"]:
            def setLcdLiveTvMode(configElement):
                open(SystemInfo["LcdLiveTVMode"], "w").write(configElement.value)
            config.lcd.live_tv_mode = ConfigSelection([("0", _("No")), ("1", _("Yes"))], "0")
            config.lcd.live_tv_mode.addNotifier(setLcdLiveTvMode)
        ...
        if SystemInfo["LcdLiveDecoder"]:
            ...
            config.lcd.live_decoder = ConfigSelection([("0", _("No")), ("1", _("Yes"))], "0")
            config.lcd.live_decoder.addNotifier(setLcdLiveDecoder)
        ...
        config.lcd.scroll_delay = ConfigSelection(default = "10000", choices = [("10000", "10 " + _("seconds")), ("20000", "20 " + _("seconds")), ("30000", "30 " + _("seconds")), ("60000", "1 " + _("minute")), ("300000", "5 " + _("minutes")), ("noscrolling", _("off"))])
