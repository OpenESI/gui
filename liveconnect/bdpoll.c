/*
Modified for OpenESI by ESI, 2025
Original code by Andreas Oberritter and David Zeuthen
See full license terms below (GNU GPL v2.0)
*/
...
static void bdpoll_notify(const char devname[])
{
	...
	if (media_status == MEDIA_STATUS_GOT_MEDIA) {
		...
		if( (end_track > 0 && media_type == CDS_AUDIO) || (end_track > 1 && media_type == CDS_MIXED) )
		{
			...
			snprintf(buf, sizeof(buf), "/usr/bin/liveconnect_e2_helper audiocdadd /dev/%s /block/%s/device 1", devname, devname);
			system(buf);
			media_mounted = false;
			audio_cd = true;
		}
		// CD/DVD will be mounted to his volume_label if avbl else to devicename
			...
			if (system(buf) == 0) {
				...
				snprintf(buf, sizeof(buf), "/usr/bin/liveconnect_e2_helper add /block/%s /block/%s/device 1", devname, devname);
				system(buf);
				media_mounted = true;
			}
			else {
		...
		if (media_mounted || audio_cd)
		{
			if (audio_cd)
			{
				snprintf(buf, sizeof(buf), "/usr/bin/liveconnect_e2_helper audiocdremove /dev/%s /block/%s/device 1", devname, devname);
				system(buf);
			...
			else
			{
				...
				snprintf(buf, sizeof(buf), "/usr/bin/liveconnect_e2_helper remove /block/%s /block/%s/device 1", devname, devname);
				system(buf);
				media_mounted = false;
			}
		}
...
static bool poll_for_media(const char devname[], bool is_cdrom, bool support_media_changed)
{
	...
	case MEDIA_STATUS_GOT_MEDIA:
		if (!got_media) {
			...
			ret = true;
			/* have to this to trigger appropriate liveconnect events */
			fd = open(device_file, O_RDONLY | O_NONBLOCK);
			if (fd >= 0) {
				ioctl(fd, BLKRRPART);
				close(fd);
