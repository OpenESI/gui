/*
LiveConnectESI – runtime module for hardware event handling in OpenESI

Based on original code by jbleyel (2024–2025)  
Derived from hotplug-e2-helper (https://github.com/OpenPLi/hotplug-e2-helper)  
Modified and maintained by OpenESI for non-commercial use
...
This file is part of the OpenESI project and contains no references or dependencies from OpenATV or OpenPLi.
*/
...
int main(int argc, char *argv[])
{
	...
	if (mode != 2)
	{
		...
		strcpy(serv_addr_un.sun_path, "/tmp/liveconnect.socket");
					...
					if (strcmp(action, "add") == 0)
					{
						if (getenv("DEVNAME"))
						{
							...
							else
							{
								...
								FILE *f;
								char fn[255];
								snprintf(fn, sizeof(fn) - 1, "/tmp/liveconnect%s", devpathnorm);
								f = fopen(fn, "w");
								if (f)
								{
									fprintf(f, data);
